...
KEEP_IF_EXISTS = ALL_ANALYSIS_BASE_COLS + OPTIONAL_EXTRA_COLS + ["FRUTO CUAJADO", "FRUTO VERDE"]
...
def load_and_prepare_data():
    ...
    required_cols = list(set(ID_COLS_DISPLAY + ["FRUTO CUAJADO", "FRUTO VERDE"] + CONTROL_COLS + [TARGET_COL]))
    ...
    numeric_cols = [
        ...
        "FRUTO CUAJADO",
        "FRUTO VERDE",
        *[c for c in PHENOLOGY_COLS if c not in ["FRUTO CUAJADO", "FRUTO VERDE", "FRUTOS TOTALES"]],
        *CONTROL_COLS,
        *OPTIONAL_EXTRA_COLS,
    ]
    df = to_numeric_safe(df, [c for c in numeric_cols if c in df.columns])

    # Crear FRUTOS TOTALES
    df["FRUTOS TOTALES"] = df["FRUTO CUAJADO"].fillna(0) + df["FRUTO VERDE"].fillna(0)
    df.loc[df["FRUTO CUAJADO"].isna() & df["FRUTO VERDE"].isna(), "FRUTOS TOTALES"] = np.nan
